// Gregorian <-> Jalaali (Persian/Shamsi) calendar conversion.
// Standard public-domain algorithm (jalaali-js), reimplemented in TypeScript.
...
export function jalaliToIso(jy: number, jm: number, jd: number): string {
  ...
  return `${gy.toString().padStart(4, '0')}-${pad2(gm)}-${pad2(gd)}`
}
...
/** Persian week starts on Saturday. Returns 0=Sat .. 6=Fri for a given Jalali date. */
export function jalaliWeekday(jy: number, jm: number, jd: number): number {
  const { gy, gm, gd } = toGregorian(jy, jm, jd)
  const gregDay = new Date(gy, gm - 1, gd).getDay() // 0=Sun..6=Sat
  return (gregDay + 1) % 7
}
