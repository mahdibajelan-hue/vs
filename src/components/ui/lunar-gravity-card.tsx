...
const RADIUS = 2.0;
const MOON_TEXTURE_URL = "https://cdn.21st.dev/assets/mirror/fc/fcb0f1f5548e6e18d40063dd55c6aacd3daedf2407b181dab85b61e22bf9fe57.jpg";

type RingState = "hidden" | "animating" | "visible";

// Every asset here (moon texture, HDR environment) is fetched from an external CDN at runtime. A
// failed fetch throws out of useTexture/Environment, and without a boundary that error unmounts
// the whole host page — so the scene degrades to "no 3D" instead of taking everything with it.
...
const AsteroidBelt = ({ ringState, massiveAsteroidsRef }: { ringState: RingState; massiveAsteroidsRef: React.RefObject<Float32Array> }) => {
  ...
  const [colorMap, bumpMap] = useTexture([MOON_TEXTURE_URL, MOON_TEXTURE_URL]);
...
export interface LunarGravityCardProps {
  ...
  /** Plays the ring/asteroid reveal on its own shortly after mount instead of waiting for a click
   * on the moon — for when the card is used as an ambient page background rather than a toy. */
  autoRevealDelayMs?: number;
  /** Extra classes for the 3D scene's container — e.g. a shorter mobile height. */
...
export default function LunarGravityCard({
  ...
  title = (
    <>
      ...
      <br />
      <span className="text-transparent bg-clip-text bg-gradient-to-b from-white via-zinc-400 to-zinc-800 drop-shadow-md">Gravity.</span>
    </>
  ),
  ...
  useEffect(() => {
    if (autoRevealDelayMs == null) return;
    const t = setTimeout(() => setRingState((s) => (s === "hidden" ? "animating" : s)), autoRevealDelayMs);
    return () => clearTimeout(t);
  }, [autoRevealDelayMs]);

  return (
    // The layout below is physical (left text / right canvas / left-anchored fade), so it's pinned
    // to LTR regardless of the host page's direction — RTL content passes its own dir inside
    // title/description.
    <div
      dir="ltr"
      className={cn(
    ...
    >
      ...
      <div className="w-full md:w-[45%] flex flex-col justify-center px-10 py-12 md:p-0 md:pl-16 relative z-20 pointer-events-none">
        <h2 className="text-[4.5rem] md:text-[5.5rem] font-bold tracking-tighter leading-[0.9] mb-6">{title}</h2>
        <div className="text-base md:text-lg text-zinc-400 font-medium leading-relaxed max-w-[340px]">{description}</div>
      </div>

      <div className={cn("relative md:absolute md:right-0 md:top-0 w-full h-[450px] md:h-full md:w-[65%] pointer-events-auto z-0 flex items-center justify-center", sceneClassName)}>
