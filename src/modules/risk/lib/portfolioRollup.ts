 ...
 * shared rasta_project_mappings table (the same mechanism Issues, PipePulse and Reporting use).
 */
export function fetchRiskProjectMappings(): Promise<Map<string, string>> {
  return fetchModuleProjectMappings('risk')
}
...
export type ProgramRollup = ReturnType<typeof buildPortfolioTree>[number]['programs'][number]
export type PortfolioRollup = ReturnType<typeof buildPortfolioTree>[number]

/** Builds the Portfolio -> Program -> Project tree with rolled-up risk KPIs at every level. */
export function buildPortfolioTree(portfolios: Portfolio[], programs: Program[], projectSummaries: ProjectRiskSummary[], masterProjects: MasterProject[]) {
  return buildHierarchyTree(portfolios, programs, masterProjects, projectSummaries, rollupTotals)
}
