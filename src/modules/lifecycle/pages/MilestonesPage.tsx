...
import { Card, EmptyState, Kpi, STATUS_COLOR, fa, faNum, faVariance } from '../components/ui'
...
export function MilestonesPage({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  const [filter, setFilter] = useState<'all' | 'problem' | 'critical'>('all')
  ...
  const rows = bundle.milestones
    ...
    .filter((r) =>
      ...
      : r.status === 'delayed' || r.status === 'at_risk' || r.status === 'blocked',
    )
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
        <Kpi label="کل" value={faNum(k.total)} />
        <Kpi label="محقق‌شده" value={faNum(k.achieved)} status="green" />
        <Kpi label="در معرض تأخیر" value={faNum(k.atRisk)} status={k.atRisk > 0 ? 'yellow' : 'green'} />
        <Kpi label="تأخیرکرده" value={faNum(k.delayed)} status={k.delayed > 0 ? 'red' : 'green'} />
        <Kpi label="بحرانی تأخیرکرده" value={faNum(k.criticalDelayed)} status={k.criticalDelayed > 0 ? 'red' : 'green'} />
        <Kpi label="تحقق به‌موقع" value={`${faNum(k.onTimeAchievementPct)}٪`} />
      </div>

      <Card
        title={`Milestoneها (${faNum(rows.length)})`}
        action={
          <div className="flex items-center gap-1">
            {(['all', 'problem', 'critical'] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className="rounded-md px-2 py-1 text-[10px] transition-colors"
                style={filter === f ? { background: '#3b82f6', color: '#fff' } : { color: 'var(--text-muted)' }}
              >
                {f === 'all' ? 'همه' : f === 'problem' ? 'دارای مشکل' : 'بحرانی'}
              </button>
            ))}
            <button onClick={() => setShowNew((v) => !v)} className="mr-1 flex items-center gap-1 rounded-md px-2 py-1 text-[10px] text-brand-400">
              <Plus size={11} /> جدید
            </button>
          </div>
        }
      >
        ...
        {rows.length === 0 ? (
          <EmptyState message="Milestoneای با این فیلتر یافت نشد" />
        ) : (
          <div className="space-y-1.5">
            {rows.map(({ ms, status, variance }) => {
              ...
              return (
                <div key={ms.id} className="rounded-lg border" style={{ borderColor: 'var(--border-soft)' }}>
                  <button
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="rounded px-1 py-px text-[8px] text-muted" style={{ background: 'var(--border-soft)' }}>
                            {MILESTONE_TYPE_LABEL_FA[ms.milestoneType]}
                          </span>
                          {ms.stageKey && (
                            <span className="text-[9px] text-muted">{STAGE_LABEL_FA[ms.stageKey as StageKey] ?? ms.stageKey}</span>
                          )}
                        </div>
                        <div className="mt-1 flex flex-wrap gap-2 text-[10px] text-muted">
                          ...
                          {ms.actualDate && <span style={{ color: STATUS_COLOR.green }}>واقعی: {fa(ms.actualDate)}</span>}
                        </div>
                      </div>
                      <div className="flex shrink-0 items-center gap-3">
                        ...
                        <div className="text-left">
                          <div className="text-xs font-bold"
                            style={{ color: (variance ?? 0) > 0 ? STATUS_COLOR.red : STATUS_COLOR.green }}>
                            {faVariance(variance)}
                          </div>
                          <div className="text-[9px]" style={{ color: STATUS_COLOR[STATUS_TO_HEALTH[status]] }}>
                    ...
                    {drift?.isWorsening && (
                      <p className="mt-1.5 flex items-center gap-1 text-[10px]" style={{ color: STATUS_COLOR.red }}>
                  ...
                  {isOpen && (
                    <div className="border-t px-3 py-2.5" style={{ borderColor: 'var(--border-soft)' }}>
                      ...
                      {linkedActions.length === 0 ? (
                        <p className="text-[10px] text-muted">اقدامی برای این Milestone ثبت نشده است</p>
                      ) : (
                      ...
                      {ms.comments && <p className="mt-2 text-[10px] text-muted">یادداشت: {ms.comments}</p>}
                    </div>
                  )}
                </div>
              )
